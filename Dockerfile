...
#
# -runtime, not -devel: 3.0GB compressed vs 6.9GB, and nothing here compiles
# CUDA. SAM 2 installs with SAM2_BUILD_CUDA=0 (skips its one optional CUDA op)
# and ProPainter is pure PyTorch. That 3.9GB is upload time on every push.
...
# --- Grounding DINO into the image's HF cache -------------------------------
RUN python -c "from transformers import AutoModelForZeroShotObjectDetection as M, AutoProcessor as P; m='IDEA-Research/grounding-dino-base'; P.from_pretrained(m); M.from_pretrained(m)"
